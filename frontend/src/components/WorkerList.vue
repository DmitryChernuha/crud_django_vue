...
export default {
  ...
  methods: {
    async getEmployee() {
      try {
        await this.$http
          .get(`http://127.0.0.1:8000/api/worker/`)
          .then((response) => {
    ...
    async submitEmployee() {
      try {
        if (!this.isEditing) {
          await this.$http
            .post(`http://127.0.0.1:8000/api/worker/`, {
          ...
          await this.$http
            .put(`http://127.0.0.1:8000/api/worker/${this.id}/`, {
              fullname: this.fullname,
              dep: this.dep,
              birthdate: this.birthdate,
              salary: this.salary,
    ...
    async deleteEmployee(employee) {
      ...
      await this.$http.delete(`http://127.0.0.1:8000/api/worker/${employee.id}/`);
